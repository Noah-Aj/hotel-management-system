...
class Roomreserve:
    def __init__(self,root):
        ...
        #########*title*########
        lbl_title=Label(self.root,text="ROOMRESERVED DETAILS",font=("times new roman",18,"bold"),bg="black",fg="gold",bd=4,relief=RIDGE)
        ...
        if self.var_contact.get()=="" or self.var_checkin.get()=="":
            ...
            if self.var_status.get()=="Clean":              
                try:
                    ...
                    my_cursor.execute("insert into reserve values(%s,%s,%s,%s,%s,%s,%s)",(
    ...
    def update(self):
        if self.var_contact.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="",database="management")
            my_cursor=conn.cursor()
            my_cursor.execute("update reserve set check_in=%s,check_out=%s,roomtype=%s,roomavailable=%s,meal=%s,noofdays=%s where contact=%s",(
    ...
    def rdelete(self):
        ...
        if rdelete>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="",database="management")
            my_cursor=conn.cursor()
            query="delete from reserve where roomavailable=%s"
